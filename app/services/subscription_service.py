...
class PlanConfiguration:
    """Plan tier configuration with limits and pricing"""
    ...
    PLANS = {
        'free': {
            ...
            'features': [
                'Status polling only',
                '5 workflow toggles per day',
                '20 list refreshes per day',
        ...
        'pro': {
            'name': 'Pro',
            'price_monthly': 19.99,
            'price_yearly': 199.99,  # ~16.67/month (16% discount)
            'toggles_per_day': 100,
            'refreshes_per_day': 200,
            'error_views_per_day': -1,  # unlimited
            'triggers': 10,
            'max_instances': 5,
        ...
        'business': {
            'name': 'Business',
            'price_monthly': 69.99,
            'price_yearly': 699.99,  # ~58.33/month (16% discount)
            'toggles_per_day': -1,  # unlimited
            'refreshes_per_day': -1,  # unlimited
            'error_views_per_day': -1,  # unlimited
            ...
            'features': [
                ...
                'Unlimited workflow toggles',
                'Unlimited list refreshes',
                'Unlimited error views',
    ...
    @classmethod
    def get_plan(cls, plan_tier: str) -> dict:
        """Get plan configuration"""
        return cls.PLANS.get(plan_tier.lower(), cls.PLANS['free'])
    
    @classmethod
    def get_limit(cls, plan_tier: str, limit_type: str) -> int:
        """Get specific limit for a plan (-1 = unlimited)"""
        plan = cls.get_plan(plan_tier)
        return plan.get(limit_type, 0)
...
class SubscriptionService:
    ...
    def get_all_plans(self) -> list[dict]:
        ...
        try:
            plans = []
            for tier, config in PlanConfiguration.PLANS.items():
                plans.append({
                    'tier': tier,
                    'name': config['name'],
                    ...
                    'recommended': tier == 'pro'  # Highlight Pro plan
                })
    ...
    def get_current_subscription(self, db: Session, user_id: str) -> dict:
        ...
        try:
            ...
            # Testers get Business tier access
            ...
            self.logger.info(f"get_current_subscription: Success - user: {user_id}, tier: {user.plan_tier}")
            return result
        except Exception as e:
            self.analytics.log_failure(
                action='get_current_subscription',
    ...
    def verify_purchase(
        ...
        self.logger.info(f"verify_purchase: Entry - user: {user_id}, tier: {plan_tier}, platform: {platform}")
        
        try:
            # Validate plan tier
            if plan_tier not in PlanConfiguration.PLANS:
            ...
            self.logger.info(f"verify_purchase: Success - user: {user_id}, subscription: {subscription.id}")
            return subscription
        except Exception as e:
            db.rollback()
            self.analytics.log_failure(
    ...
    def cancel_subscription(self, db: Session, user_id: str) -> Subscription:
        ...
        try:
            ...
            self.logger.info(f"cancel_subscription: Success - user: {user_id}, subscription: {subscription.id}")
            return subscription
        except Exception as e:
            db.rollback()
            self.analytics.log_failure(
    ...
    def get_subscription_history(self, db: Session, user_id: str) -> list[dict]:
        ...
        try:
            ...
            self.logger.info(f"get_subscription_history: Success - user: {user_id}, count: {len(result)}")
            return result
        except Exception as e:
            self.analytics.log_failure(
                action='get_subscription_history',
    ...
    def check_expired_subscriptions(self, db: Session) -> int:
        ...
        try:
            ...
            self.logger.info(f"check_expired_subscriptions: Success - expired: {count}")
            return count
        except Exception as e:
            db.rollback()
            self.analytics.log_failure(
