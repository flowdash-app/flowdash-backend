...
class VerifyPurchaseRequest(BaseModel):
    ...
    platform: str  # 'google_play' or 'apple_store'
    purchase_token: str
...
async def get_current_subscription(
    ...
    try:
        subscription = subscription_service.get_current_subscription(db, user_id)
...
async def verify_purchase(
    ...
    """
    user_id = current_user['uid']
    logger.info(f"verify_purchase: Entry - user: {user_id}, tier: {request.plan_tier}")
    ...
    try:
        ...
        logger.info(f"verify_purchase: Success - user: {user_id}, subscription: {subscription.id}")
        return {
...
async def cancel_subscription(
    ...
    try:
        ...
        logger.info(f"cancel_subscription: Success - user: {user_id}, subscription: {subscription.id}")
        return {
            "subscription_id": subscription.id,
            "status": subscription.status.value,
            "end_date": subscription.end_date.isoformat() if subscription.end_date else None,
...
async def get_subscription_history(
    ...
    try:
        ...
        logger.info(f"get_subscription_history: Success - user: {user_id}, count: {len(history)}")
